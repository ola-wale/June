//controller for the experience view.
app.controller('experienceController', function($scope,$rootScope) {
	var chart = document.getElementById('experienceChart').getContext('2d');
	...
	var labels = ['Earbook Inc.', 'Foogle', 'ActiviDuty'];
	//must be unique!!!
	var dataArr = [2009, 2012, 2016];
	var descriptionObj = {
		0: {
			title: 'Web Designer',
			subTitle: 'Earbook Inc.',
	...
	$("#experienceChart").mousemove(function(e) {
		...
		if (typeof index != 'undefined') {
			...
			var adjustCanvases = function() {
				ctx.canvas.width = document.getElementById('experienceChart').getContext('2d').canvas.width;
				ctx.canvas.height = document.getElementById('experienceChart').getContext('2d').canvas.height;
			}
			adjustCanvases();
	...
	var ctxShadow = document.getElementById("experienceShadowChart").getContext("2d");;
